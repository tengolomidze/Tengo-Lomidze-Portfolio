...
import PixelPaint from './pages/PixelPaint';
...
const Window = (props:any) => {
    ...
    const getPage = (name:string) => {
        switch(name){
            ...
            case "Pixel Paint.exe":
                return <PixelPaint window={props.windows[props.i]}/>
            default: 
                return <></>
        }
    }
    ...
    return (
        <>
            ...
            >
                ...
                <div onClick={handleClick} style={{width: props.windows[props.i].size.x, height: props.windows[props.i].size.y, zIndex: props.z }} className='flex flex-col items-start justify-center fixed bg-gray-100 dark:bg-zinc-900 rounded-md border-2 border-zinc-400 dark:border-zinc-600 overflow-auto' >
                    <div className='h-8 w-full bg-white dark:bg-black flex items-center justify-center '>
                        <div className='w-full h-full handle relative  z-10 flex p-1 items-center justify-start select-none'>
                            ...
                            <p className='text-xs ml-2 dark:text-zinc-200'>C:\{props.name}</p>
                        </div>
                        ...
                        <div className='h-full flex items-center justify-center'>
                            ...
                            {
                                props.windows[props.i].size.x >= window.innerWidth ? 
                                    <div onClick={handleMinimize} className='hover:bg-zinc-300 dark:hover:bg-zinc-700 dark:bg-opacity-20 rounded-sm bg-opacity-20 duration-200 h-full flex items-center justify-center px-2'>
                                        <VscChromeMinimize   className="text-md  text-gray-600 dark:text-gray-200"/>
                                    </div>
                                :
                                    <div onClick={handleFullScreen} className='hover:bg-zinc-300 dark:hover:bg-zinc-700 dark:bg-opacity-20 rounded-sm bg-opacity-20 duration-200 h-full flex items-center justify-center px-2'>
                                        <FiMaximize  className="text-md   text-gray-600 dark:text-gray-200"/>
                                    </div>
                            }
                           

                            <div onClick={handleClose}  className='hover:bg-zinc-300 dark:hover:bg-zinc-700 dark:bg-opacity-20 rounded-sm  bg-opacity-20 duration-200 h-full flex items-center justify-center px-2'>
                                <VscChromeClose  className="text-lg text-gray-600 dark:text-gray-200"/>
                            </div>
                        </div>
                    </div> 
